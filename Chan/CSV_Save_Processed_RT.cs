...
public class CSV_Save_Processed_RT : MonoBehaviour
{
    ...
    string[] csvHeaders = new string[] { "Condition", "TaskCount", "ImageOrder", "PlayerAnswer", "IsCorrect", "AnsweringTimer" };
    ...
    public void Save_CSV_Processed()
    {
        ...
        PlayerData[2] = RT.ImageOrder; // 1이면 원본 먼저, 0이면 foveation 먼저
        PlayerData[3] = RT.PlayerAnswer; // 0이면 먼저 나온 이미지 선택, 1이면 뒤에 나온 이미지 선택
        PlayerData[4] = RT.IsCorrect; // 1이면 정답, 0이면 오답
