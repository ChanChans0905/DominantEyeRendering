...
public class ImageController : MonoBehaviour
{
    ...
    public void RT_TurnOnImage()
    {
        ...
        if (CO == 0)
            TurnOnImage(RTI_S1, TO);
        else if (CO == 1)
            TurnOnImage(RTI_S2, TO);
        else if (CO == 2)
            TurnOnImage(RTI_D1, TO);
        else if (CO == 3)
            TurnOnImage(RTI_D2, TO);
    }

    void TurnOnImage(GameObject[] ImageGroup, int TaskOrder)
    {
        ImageGroup[0].layer = RT.ImageOrder == 0 ? Layer_Left : Layer_Right;

        for (int i = 1; i < 5; i++)
            ImageGroup[i].layer = RT.ImageOrder == 0 ? Layer_Right : Layer_Left;

        ImageGroup[0].SetActive(true);
        ImageGroup[TaskOrder / 2].SetActive(true);
    }
    ...
    public void ST_TurnOnOffImage(bool TurnOnOff, int SC)
    {
        ...
        if (CO == 0)
        {
            ST_AdjustImageLayer(STI_S1, ST.DominantEye);
            STI_S1_100.SetActive(TurnOnOff);
            STI_S1[SC].SetActive(TurnOnOff);
        }
        else if (CO == 1)
        {
            ST_AdjustImageLayer(STI_S2, ST.DominantEye);
            STI_S2_100.SetActive(TurnOnOff);
            STI_S2[SC].SetActive(TurnOnOff);
        }
        else if (CO == 2)
        {
            ST_AdjustImageLayer(STI_D1, ST.DominantEye);
            STI_D1_100.SetActive(TurnOnOff);
            STI_D1[SC].SetActive(TurnOnOff);
        }
        else if (CO == 3)
        {
            ST_AdjustImageLayer(STI_D2, ST.DominantEye);
            STI_D2_100.SetActive(TurnOnOff);
            STI_D2[SC].SetActive(TurnOnOff);
        }
    }

    void ST_AdjustImageLayer(GameObject[] ImageGroup, int LorR)
    {
        ImageGroup[0].layer = LorR == 0 ? Layer_Left : Layer_Right;
        for (int i = 1; i < 5; i++)
